...
#spark= SparkSession.builder.appName("HEDISAbstractor.AI").getOrCreate()

server=os.getenv('HEDIS_DATABRICKS_SERVER')
sql_server_name = os.getenv('HEDIS_DATABRICKS_SQL_SERVER')
catalog_name = os.getenv('HEDIS_DATABRICKS_CATALOG')
database_name = os.getenv('HEDIS_DATABRICKS_DATABASE')
db_token = os.getenv('DBX_TOKEN')
...
def get_db_connection(http_path):
    return sql.connect(
...
def create_connection(http_path):
    return sql.connect(
       server_hostname=server,
        http_path=sql_server_name,
        access_token=db_token,
...
# db_config = {
#     'Catalog': os.getenv('HEDIS_DATABRICKS_CATALOG'),
#     'Database': os.getenv('HEDIS_DATABRICKS_DATABASE'),
# }


# # Validate db_config
# if not db_config['Catalog'] or not db_config['Database']:
#     st.error("Missing HEDIS_DATABRICKS_CATALOG or HEDIS_DATABRICKS_DATABASE in .env file.")
#     st.stop()


conn = create_connection(sql_server_name)
# CSS for UI
st.markdown(
    """
    <style>
...
def get_confidence_score(measure, filename):
    ...
    query=f"select * from {catalog_name}.{database_name}.{confidence_table} WHERE measure = upper('{measure}') AND filename = '{filename}'"
    df= database.sqlQuery(query, db_token)
    return dict(zip(df['measure'], df['score']))
...
# def create_connection():
#     try:
#         spark.sql(f"USE CATALOG {db_config['catalog']}")
#         spark.sql(f"USE {db_config['Database']}")
#         return spark
...
#AK
def get_member_id_via_pdf_old(file_name):
    file_name=file_name.strip(' ')

    file_name= file_name+'.pdf' if not file_name.endswith('.pdf') else file_name

    engine=create_connection(http_path)
    ...
    if engine:
        try:
            ...
            st.error(f"Query execution error: {str(e)}", icon="🚨")
            return None
    else:
        return None
...
def get_member_id_via_pdf(file_name):
    ...
    query=f'select FileID from {catalog_name}.{database_name}.{file_info_table} where Pdf_filename='+f"'{file_name}' AND is_active=True"
...
def getdata(member_id, table):
    ...
    conn = create_connection(http_path)
    if conn:
        try:
            if table == 'memberinfo':
                a=member_id.split('.')[0].split('_')[-1]
            
                query = f"SELECT * FROM {catalog_name}.{database_name}.{table} WHERE member_id = {a} AND is_active =True"
            else:
                query = f"SELECT * FROM {catalog_name}.{database_name}.{table} WHERE FileID = '{member_id}' AND is_active = True"
            # st.write(query)
            data= database.sqlQuery(query, db_token)
            # st.dataframe(data=data, height=600, use_container_width=True)
    ...
    if conn:
        try:
            if search_type == 'File Name':
                query = f"SELECT FileID as Member_id, Pdf_filename as Name FROM {catalog_name}.{database_name}.{file_info_table} WHERE FileID ='{search_value}' AND is_active IN (1)"
            else:
                query = f"SELECT * FROM {catalog_name}.{database_name}.{table} WHERE Member_id = {search_value} AND is_active IN (1)"
...
def getdata3(search_value, table, search_type='Member_id'):
    ...
    query = f"""
                        ...
                        FROM {catalog_name}.{database_name}.{file_info_table} f
                        RIGHT JOIN {catalog_name}.{database_name}.{member_info_table} m ON cast(f.FileID as int) = cast(m.Member_id as int) 
    ...
    if not data.empty:
        ...
        # st.write("EMTY")
        # st.warning(f"No records are found for member_id {search_value}")
        return pd.DataFrame()
...
def lookup_pdf_location(member_id):
    try:
        ...
        FROM {catalog_name}.{database_name}.{file_info_table}
...
def update_multiple_columns(table, member_id, updates):
    ...
    set_clauses = [f"{k} = '{v}'" if isinstance(v, str) else f"{k} = {v}" for k, v in updates.items()]
    set_clause = ", ".join(set_clauses) + ", updated_date = GETDATE()"
    query = f"UPDATE {catalog_name}.{database_name}.{table} SET {set_clause} WHERE FileID ='{member_id}' AND is_active = True" #AYU added FIleID
    # st.write(query) 
    ...
    #     finally:
    #         conn.close()
    database.executeQuery(query, db_token)
...
def display_pdf(file_name):
    query=f"select filename, content from {catalog_name}.{database_name}.{pdf_binary_table_name} where filename='{file_name}'"
#     st.download_button(
#     label="Download PDF",
#     data=pdf_data,
...
# )
    ...
    pdf_data=database.sqlQuery(query, db_token)
    pdf_data = pdf_data["content"].values[0] #pdf_data[pdf_data["filename"] == selected_file]["content"].values[0]
...
st.markdown(
    """
    <div style="background: linear-gradient(135deg, rgba(251, 78, 11, 0.05), rgba(255, 107, 53, 0.05)); 
                padding: 20px; border-radius: 15px; border: 1px solid rgba(251, 78, 11, 0.1);
                margin-bottom: 25px; text-align: center; font-size: 16px; font-weight: 500;">
        Please enter the Member ID/File Name and select the Edit button to update the data.
    </div>
    """,
    unsafe_allow_html=True
)
...
col1, col2, col3 = st.columns([0.5, 0.75, 2])
...
if search_value:
    ...
    if search_option == 'File Name':
        ...
        # st.write(f"{search_value}, {type(search_value)}")
        # st.dataframe(data=data, height=600, use_container_width=True)
    ...
    if data.empty:
        ...
        if grid_response['data'].__action__.eq('edit').any():
            ...
            st.session_state['pdf_file_name']=selected_row['File Name']
            ...
            st.session_state['compliance_status'] = {}
            # st.session_state['page_number'] = 1
            # st.write("LOG2:PgNO-",st.session_state['page_number'])
            st.session_state['field_compliance'] = {}
            st.session_state['show_confirmation'] = {}
            st.session_state['pending_updates'] = {}
            st.session_state['pending_measure'] = None
        ...
        #     st.session_state['pdf_file_name']=row['File Name']
    ...
    if 'edit_index' in st.session_state and st.session_state['edit_index']:
        ...
        # pdf_location = "/Volumes/aiagenticdemo/hedisabstractor/landing_pdf/Linda_Test_Record_2420.pdf" #lookup_pdf_location(st.session_state['edit_index']) #AYU Passing the file name using session state
        pdf_file_name=st.session_state.get('pdf_file_name')
        # st.write(f"pdf_location: database.{pdf_file_name}")
        pdf_document = None
        #member_id=search_value
        ...
        # if pdf_file_name:
        #     pdf_binary_data=get_pdf_binary_data(pdf_file_name)
            
        #     if pdf_binary_data is None:
        #         with right_column:
        #             with st.container(height=775, border=True):
        #                 st.warning(f"File not found {pdf_file_name}",icon="⚠️")
        #     else:
        #         with right_column:
        ...
        with left_column:
            with st.container(height=775, border=True):
                ...
                st.markdown("""
                <style>
                /* Make Streamlit tab labels bold and improve appearance */
                ...
                with tabs[0]:
                    ...
                    if not data1.empty:
                        ...
                        with cols[0]:
                            confidence_score= get_confidence_score('generic', filename).get('generic', '89.2%')
                ...
                for measure in available_tabs[1:]:
                    confidence_score= get_confidence_score(measure, filename).get(measure,'89.26%')
                    confidence_score= f"{float(confidence_score.rstrip('%')):.2f}%"
                    ...
                    with tabs[available_tabs.index(measure)]:
                        ...
                        with cols[1]:
                            ...
                            if st.button("ℹ️", key=f"info_{measure}", help=f"View/Close {measure} specifications"):
            ...
            if pdf_binary_data is None:
                ...
                with right_column:
                    ...
                    with st.container(height=775, border=True):
                        ...
                        page_input = st.session_state['page_number']
                        if page_input != st.session_state['page_number']:
                            st.session_state['page_number'] = page_input
                        st.markdown('</div>', unsafe_allow_html=True)
                        display_pdf_page(st.session_state['page_number'] - 1, pdf_document)
