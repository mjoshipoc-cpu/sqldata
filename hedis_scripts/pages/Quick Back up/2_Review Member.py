...
def display_pdf(file_path):
    ...
    with open(file_path, "rb") as f:
        ...
        pdf_display = f'<iframe src="data:application/pdf;base64,{base64_pdf}#page={a}" width="570" height="1425" type="application/pdf"></iframe>'
...
#     query = f"SELECT * FROM {table} WHERE Member_id = '{member_id}' and is_active in (0,1)"  
#     return pd.read_sql(query, conn)


def getdata(search_value, table, search_type='Member_id'):
    conn = create_connection()
    if search_type == 'Name':
        query = f"SELECT * FROM {table} WHERE Name LIKE '%{search_value}%'"
    else:
        query = f"SELECT * FROM {table} WHERE Member_id = '{search_value}'"
    return pd.read_sql(query, conn)
...
def lookup_pdf_location(member_id):
    try:
        ...
        conn = create_connection()
        query = f"SELECT FileID FROM memberinfo WHERE member_id = {member_id} and is_active in (0,1)"
        result1 = pd.read_sql(query, conn)
        if not result1.empty:
            file_id = result1.iloc[0]['FileID']
            query = f"SELECT concat(Pdf_location,'\\',Pdf_filename) as loc FROM file_info WHERE FileID = '{file_id}' and is_active in (0,1)"
...
def create_new_dataframe(input_df):
    # Step 1: Get all columns up to the 10th column
    columns_till_10 = input_df.columns[:10]

    # Step 2: Get alternate columns after the 10th column
    alternate_columns = input_df.columns[11::2]

    # Step 3: Combine both sets of columns
    selected_columns = list(columns_till_10) + list(alternate_columns)
...
def page_df(input_df):
    alternate_columns = input_df.columns[10::2]
    selected_columns = list(alternate_columns)
    new_df = input_df[selected_columns]
    l2=new_df.iloc[0].tolist()
    l1= ["1" for i in range(9)]
    l3=l1+l2
    return l3
            ...
            with left_column:
                ...
                # Create a form for editing
                    with st.form("edit_form"):
                        tabs = st.tabs(["**General**","**BCS**", "**CBP**", "**HBD**","**BPD**"])
                        with tabs[0]:
                            data = getdata(search_member_id,"memberinfo")
                        ...
                        with tabs[1]:
                            data = getdata(search_member_id,"bcs")
                            #st.write(data)
                            data1=create_new_dataframe(data)
                            pagedf=page_df(data)
                            #st.write(pagedf)
                            if not data1.empty:
                                updated_bcs = {}
                                #st.write(len(data1.columns))
                                for col,val  in zip(data1.columns,pagedf):
                                    #updated_values[col] = st.text_input(f"**{col}**",help="Page No.1", value=str(row_to_edit[col]))
