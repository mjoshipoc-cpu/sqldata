...
def create_new_dataframe(input_df, measure):
    ...
    elements_to_remove = ["Name", "Member_id", "FileID", "is_active", "Remark","Updated_date","Insert_Date"]
    ...
    if not display_columns:
        ...
        return pd.DataFrame()
    return input_df[display_columns]

def create_new_dataframe_meminfo(df):
    #page_columns = [col for col in df.columns if 'page' in col]
    non_page_columns = [col for col in df.columns if 'page' not in col.lower() and col not in ["Member_id", "FileID", "is_active", "Remark","Updated_date","Insert_Date"]]
                ...
                with tabs[0]:
                    ...
                    if not data1.empty:
                        ...
                        for idx, (filter_col,col, val) in enumerate(zip(filter_col,data1.columns, pagedf + [None] * (len(data1.columns) - len(pagedf)))):
                            cols = st.columns([0.85, 0.15, 0.08])
                            with cols[0]:
                ...
                for measure in available_tabs[1:]:
                    ...
                    with tabs[available_tabs.index(measure)]:
                        ...
                        if not data1.empty:
                            ...
                            for idx, (filter_col,col, val) in enumerate(zip(filter_col,data1.columns, pagedf)):
                                ...
                                # with cols[2]:
                                #     # SYN:Removed: Compliance status
                                #     # compliance_remark = check_field_compliance(member_id, measure, updated_data, member_info_df, col)
                                #     # st.session_state['compliance_status'][f"{col}_{measure}"] = compliance_remark
                                #     # status_class = get_compliance_status(compliance_remark)
                                #     # st.markdown(
                                #     #     f'<div class="tooltip"><span class="status-badge {status_class}" onclick="document.getElementById(\'modal-{col}-{measure}\').classList.toggle(\'show\')">{status_class.split("-")[1].capitalize()}</span><span class="tooltiptext">{compliance_remark}</span></div>',
                                #     #     unsafe_allow_html=True
                                #     # )
                                #     # st.markdown(
                                #     #     f"""
                                #     #     <div id="modal-{col}-{measure}" class="modal">
                                #     #         <div class="modal-header">
                                #     #             <span>{col} Compliance Details</span>
                                #     #         </div>
                                #     #         <div class="modal-content">
                                #     #             <strong>Measure Name:</strong> {measure}<br><br>
                                #     #             <p><strong>Current Remark:</strong> {compliance_remark}</p>
                                #     #         </div>
                                #     #     </div>
                                #     #     """,
                                #     #     unsafe_allow_html=True
                            ...
                            with col1:
                                #synd Removed: Preview Compliance button
                                # if st.button("Preview Compliance", type='primary',  key=f"preview_{measure.lower()}"):
                                #     if "Non-compliant" in overall_compliance_remark:
                                #         st.markdown(f'<div class="non-compliant-remark">Compliance Status: {overall_compliance_remark}</div>', unsafe_allow_html=True)
                                #     else:
                                #         st.markdown(f'<div class="compliance-remark">Compliance Status: {overall_compliance_remark}</div>', unsafe_allow_html=True)
                                pass
                            with col2:
