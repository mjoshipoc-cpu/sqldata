...
def filter_df(df,flag):
    ...
    if flag:
        ...
        elements_to_remove = ["is_active","Insert_Date"]
    else:
        elements_to_remove = ["Name","FileID","Update_Date","Insert_Date","is_active","Remark"]
        ...
        init(params) {
            ...
            this.eGui.innerHTML = `
             <span>
                ...
                <button class="btn_view">👁️ Review</button>
             </span>
          `;
          // Attach click -> set hidden action cell so Python can read it
          const btn = this.eGui.querySelector('button');     // ---NEW CHANGE---
          btn.addEventListener('click', () => {              // ---NEW CHANGE---
              try {                                          // ---NEW CHANGE---
                  this.params.node.setDataValue('__action__', 'view'); // ---NEW CHANGE---
              } catch (e) { console.error(e); }              // ---NEW CHANGE---
          });                                                // ---NEW CHANGE---
        }
        ...
        init(params) {
            ...
            this.eGui.innerHTML = `
             <span>
                ...
                <button class="btn_summary">📋</button>
             </span>
          `;
          // Attach click -> set hidden action cell so Python can read it
          const btn = this.eGui.querySelector('button');     // ---NEW CHANGE---
          btn.addEventListener('click', () => {              // ---NEW CHANGE---
              try {                                          // ---NEW CHANGE---
                  this.params.node.setDataValue('__action__', 'summary'); // ---NEW CHANGE---
              } catch (e) { console.error(e); }              // ---NEW CHANGE---
          });                                                // ---NEW CHANGE---
        }
...
# (No need for separate onCellClicked JS anymore; we mutate a hidden field from the renderer)  
...
def create_dynamic_grid_config(dataframe):
    ...
    # Hide the helper action column from UI
    gb.configure_column('__action__', hide=True, editable=False, filter=False, sortable=False)  
...
def main():
    ...
    print(f"data1 columns: {data1.columns}")
    measures = ["All","BCS", "CBP", "HBD", "BPD", "EED", "CCS", "COL", "LSD", "PPC"]
    ...
    rearranged_columns = []
    first_column = ['Measurement_Year','Insert_Date']
    ...
    # Ensure required fields exist even if not in the first 6 columns (hide them later)  
    for req in ['Name','Member_id','FileID']:                    
        if req not in data_display.columns and req in data.columns:  
            data_display[req] = data[req]                         


    # ---MODIFY---- Add button columns to dataframe for ag-grid
    data_display['Review'] = ''                                     
    data_display['Summary'] = ''                                  
    data_display['__action__'] = ''                               
    ...
    # ---MODIFY---- Handle action signaled from JS via the hidden column
    df_after = grid_response['data']                               
    try:                                                            
        action_rows = df_after[df_after['__action__'].isin(['view','summary'])]  
        if not action_rows.empty:                                   
            act_row = action_rows.tail(1).iloc[0]                   
            st.session_state['member_name'] = act_row.get('Name', '')            
            st.session_state['member_id'] = act_row.get('Member_id', '')         
            st.session_state['FileID'] = act_row.get('FileID', '')               
            if act_row['__action__'] == 'view':                                    
                st.switch_page("pages/2_ReviewMember.py")                         
            else:                                                                  
                st.session_state['files_name'] = st.session_state['FileID']       
                st.switch_page("pages/5_Summarization.py")                        
    except Exception as e:                                                         
        st.warning(f"Button handler error: {e}")                                   
